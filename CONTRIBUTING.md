...
idea in this package. It declares eight drawing primitives:
...
- **The trait is deliberately narrow.** No `draw_ellipse`, no
  `fill_polygon`, no dashes, clipping, radial gradients, or path-shaped
  gradients. Each of those exists in the package as a free function or
  `Canvas` method; none has a concrete caller *through the trait*. Add
  to the trait when something concrete needs it, not before — every
  addition is a method both backends must implement forever.
