...
alias, resolving to DejaVu Sans on this machine -- same real-machine
dependency test_glyph_outline.mojo/test_font_discovery.mojo already
document) to run.
...
def test_cmap_format4_and_format12_agree_on_known_codepoints() raises:
    # DejaVu Sans exposes both a format 4 (BMP-only) and format 12
    # (full Unicode) cmap subtable for the same characters (confirmed
    # via probe: platform 0/encoding 3 and platform 3/encoding 1 both
    # point at one format-4 subtable; platform 0/encoding 4 and
    # platform 3/encoding 10 both point at one format-12 subtable) --
    # two independently-decoded subtables agreeing exactly on the same
    # glyph index is a real cross-check that both decoders are correct,
...
def test_missing_codepoint_maps_to_notdef() raises:
    # 0x10FFFE -- the last valid Unicode codepoint, confirmed via the
    # same Python oracle to fall outside every group in this font's own
    # format-12 subtable.
    var face = _sans_face()
...
def test_composite_glyph_matches_independently_computed_points() raises:
    # 'é' (U+00E9) -- a real composite glyph in DejaVu Sans (confirmed
    # via probe: numberOfContours == -1, i.e. references component
    # glyphs rather than describing its own outline directly), built
    # from a base "e" plus an offset+scaled acute-accent component.
    # Every point below was independently computed by a from-scratch
    # Python oracle (plain struct.unpack, no font libraries -- see this
    # file's own module docstring) implementing the exact same
    # component-transform math (OpenType spec's own `x' = xscale*x +
    # scale10*y` formula) separately from this Mojo code, then diffed
    # byte-for-byte against this module's actual output before being
    # locked in here -- real cross-validation of the composite-glyph
    # path (component offsets, scale application, point renumbering
    # across multiple components), not just "it produced some points".
    var face = _sans_face()
    ...
    # Spot-check specific points from each of the three contours
    # (base "e"'s outer contour, "e"'s inner hole, and the accent mark)
    # rather than re-embedding all 32 -- enough to catch a real
    # regression in either the base glyph decode or the component
    # transform/offset math specifically.
...
def test_advance_width_matches_hmtx() raises:
    # 'O's advance width, in raw font-design units -- cross-checked via
    # probe against the same Python oracle used for the composite-glyph
    # test above (reading the same hmtx LongHorMetric record directly).
    var face = _sans_face()
    var gid = face.glyph_index_for_codepoint(0x4F)
