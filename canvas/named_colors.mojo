"""Named colors: the CSS Color Module Level 3 / X11 "extended color
keywords" list (<https://www.w3.org/TR/css-color-3/#svg-color>) plus
`REBECCAPURPLE` (Level 4), as `Color` constants, so a caller can write
`CORNFLOWERBLUE` instead of `Color(100, 149, 237)`. Each name is the
CSS keyword uppercased with no separators (`CORNFLOWERBLUE`, not
...
`comptime` constants, usable anywhere a `Color` literal is, with no
runtime lookup. Sorted alphabetically by name, matching the CSS spec's
own listing. Re-exported from the package root, so
`from canvas import CORNFLOWERBLUE` resolves.
"""
